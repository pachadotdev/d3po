...
module.exports = (vars) ->
  ...
  if vars.timeline.value and (!vars.error.internal or !vars.data.missing) and !vars.small and vars.data.time and vars.data.time.values.length > 1
    ...
    playButton = vars.g.timeline.selectAll("rect.d3po_timeline_play")
    ...
    playIcon = vars.g.timeline.selectAll("text.d3po_timeline_playIcon")
      .data if tallEnough and vars.timeline.play.value then [0] else []
    ...
    background = vars.g.timeline.selectAll("rect.d3po_timeline_background")
      .data(["background"])
    ...
    if vars.timeline.handles.value
      ...
      handles = brush_group.selectAll("g.resize").selectAll("rect.d3po_handle")
        .data(["d3po_handle"])
