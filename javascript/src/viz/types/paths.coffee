...
viz = (vars) ->
  ...
  for node in vars.data.viz
    ...
    if vars.size.value
      val = fetchValue vars, node, vars.size.value
      node.d3po.r = if val then size(val) else minRadius
    else
      node.d3po.r = maxRadius

    if node.d3po.r < columnWidth * 0.1 and !vars.small
      node.d3po.label =
        x: 0
  ...
  for path, pathInt in viz.paths
    ...
    for edge, edgeInt in path.edges
      ...
      if pathLookup[lastHop] isnt pathLookup[nextHop]
        ...
        edge[vars.edges.source].d3po ?= {}
        edge[vars.edges.source].d3po.edges ?= {}
        edge[vars.edges.target].d3po ?= {}
        edge[vars.edges.target].d3po.edges ?= {}
