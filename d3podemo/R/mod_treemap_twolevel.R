...
#' Two-level Treemap (drillable)
#' @param data data.frame
#' @return d3po widget
mod_treemap_twolevel_plot <- function(data = d3po::pokemon) {
  ...
  d3po(dout, width = 800, height = 600) %>%
    po_treemap(daes(size = .data$count, group = .data$type1, subgroup = .data$type2, color = .data$color, tiling = "squarify")) %>%
