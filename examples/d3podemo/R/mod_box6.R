...
#' @return d3po widget
mod_box6_plot <- function(data = d3po::pokemon) {
  ...
  d3po(data, width = 800, height = 600) %>%
    po_box(daes(x = .data$type_1, y = .data$weight, color = my_pal)) %>%
    po_labels(title = "Weight Distribution by Type")
}

mod_box6_ui <- function(id) {
  ns <- NS(id)
  tagList(d3po::d3po_output(ns("plot"), width = "100%", height = "600px"))
}

mod_box6_server <- function(id, data = d3po::pokemon) {
