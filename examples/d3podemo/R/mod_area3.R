# Module: area3 (stacked)
#' Area chart: stacked example
#' @param data data.frame
#' @return d3po widget
mod_area3_plot <- function(data = d3po::pokemon) {
  ...
  d3po(dout, width = 800, height = 600) %>%
    po_area(
      daes(
        x = .data$stage, y = .data$value, group = .data$feature, stack = T
      )
    ) %>%
    po_labels(title = "Pokemon Evolution (Squirtle): Defense/Attack Proportion by Evolution Stage (Stacked Area)")
}

mod_area3_ui <- function(id) {
  ns <- NS(id)
  tagList(d3po::d3po_output(ns("plot"), width = "100%", height = "600px"))
}

mod_area3_server <- function(id, data = d3po::pokemon) {
