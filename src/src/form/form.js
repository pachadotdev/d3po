...
module.exports = () => {
  ...
  vars.self = () => {
    ...
    if (vars.data.value instanceof Array) {
      ...
      //------------------------------------------------------------------------
      if (
        vars.data.sort.value &&
        (vars.data.changed || vars.order.changed || vars.order.sort.changed)
      ) {
        arraySort(
          vars.data.viz,
          vars.order.value || vars.text.value,
      ...
      if (vars.type.value !== 'auto') {
        ...
        // Update Container
        //------------------------------------------------------------------------
        vars.container.ui.style('display', vars.ui.display.value);
        ...
        title
          .enter()
          .insert('div', '#d3po_' + vars.type.value + '_' + vars.container.id)
