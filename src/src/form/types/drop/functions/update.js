...
const scrolllist = require('./scroll.js');
...
module.exports = vars => {
  ...
  function position(elem) {
    const flipped = vars.open.flipped.value;
    elem
      .style('top', () =>
    ...
    elem
      .style('left', () => {
        if (vars.font.align.value === 'left') {
          ...
          return (
            vars.margin.left -
            (vars.width.secondary - vars.width.value) / 2 +
            'px'
          );
        } else {
          return 'auto';
  ...
  function finish(elem) {
    elem.style('display', vars.open.value ? null : 'none').call(position);

    if (vars.search.enabled && vars.open.value) {
      vars.container.selector
        .select('div.d3po_drop_search input')
        .node()
        .focus();
    }
  }

  const max_height = vars.open.value
    ? vars.height.secondary - vars.margin.title
    : 0;
