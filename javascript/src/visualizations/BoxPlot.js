...
export default class BoxPlot extends D3po {
  ...
  render() {
    ...
    const formattedCols =
      this.options && this.options.formattedCols
        ? this.options.formattedCols
        : null;
    ...
    // needs, using a temporary probe scale/axis, before any box shapes are
    // drawn. This avoids wasting space with the base class's generous
    // default bottom margin (sized for the worst case of rotated labels).
    {
      const probeDomain = isHorizontal
        ? [d3.min(boxData, d => d.stats.min), d3.max(boxData, d => d.stats.max)]
        : boxData.map(d => d.label);
      const probeInnerWidth = this.getInnerWidth();
      const probeScale = isHorizontal
        ? d3
            .scaleLinear()
            .domain(probeDomain)
            .nice()
            .range([0, probeInnerWidth])
        : d3
            .scaleBand()
            .domain(probeDomain)
            .range([0, probeInnerWidth])
      ...
      try {
        ...
        if (
          this.options &&
          this.options.axisFormatters &&
          this.options.axisFormatters.x
        ) {
        ...
        if (probeTicks && probeTicks.length) {
          ...
          if (probeScale.bandwidth) {
            ...
            const avgSpace = Math.max(10, probeInnerWidth / probeTicks.length);
    ...
    if (isHorizontal) {
      ...
      if (
        ...
        bottomAxis.tickFormat(this.options.axisFormatters.x);
      ...
      if (
        this.options &&
        this.options.axisFormatters &&
        this.options.axisFormatters.y
      )
        leftAxis.tickFormat(this.options.axisFormatters.y);
    }
    ...
    try {
      ...
      try {
        const lab = this.svg
          ...
          .text(isHorizontal ? this.xField : this.yField);
      ...
      if (requiredLeft > currentLeft) {
        ...
        if (categoryScale && categoryScale.range) {
          if (isHorizontal) categoryScale.range([0, effectiveInnerHeight]);
          else categoryScale.range([0, effectiveInnerWidth]);
        }
        if (valueScale && valueScale.range) {
          if (isHorizontal) valueScale.range([0, effectiveInnerWidth]);
          else valueScale.range([effectiveInnerHeight, 0]);
        }
      ...
      .on('mouseover', function (event, d) {
        ...
        if (tooltipFormatter) {
          ...
          showTooltipWithFormatter(
            ...
            tooltipFormatter,
            null,
            { group: d.group, label: d.label || d.group, stats: d.stats },
            fontFamily,
            fontSize,
            fallback
          );
