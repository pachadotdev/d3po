    ...
    if (vars.draw.update) {
      ...
      (!vars.nodes.positions || vars.nodes.changed || vars.type.changed)
        ...
        if (
            vars.data.changed ||
      vars.time.changed ||
      vars.time.format.changed ||
      vars.type.changed ||
      vars.id.changed ||
      (vars.x.scale.changed &&
        [vars.x.scale.value, vars.x.scale.previous].indexOf('discrete') >= 0) ||
      (vars.y.scale.changed &&
        [vars.y.scale.value, vars.y.scale.previous].indexOf('discrete') >= 0)
        ) {
            steps.push({
                function: dataFormat,
                message: dataMessage
        ...
        if (!vars.error.value) {
            steps.push({
                function: function(vars) {
                    ...
                    vars.draw.timing =
            vars.data.viz.length < vars.data.large
