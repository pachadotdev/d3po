(function() {
    ...
    module.exports = function(vars, opts) {
        ...
        vars.axes.scale =
      opts.buffer && opts.buffer !== true
          ? sizeScale(vars, opts.buffer)
          : false;
        axes =
      vars.width.viz > vars.height.viz
        ...
        for (i = 0, len = axes.length; i < len; i++) {
            ...
            reorder =
        vars.order.changed ||
        vars.order.sort.changed ||
        (vars.order.value === true && vars[oppAxis].changed);
            if (
                vars[axis].value &&
        (!vars[axis].ticks.values ||
          changed ||
          reorder ||
          (vars[axis].value === vars.time.value && vars.time.fixed.value))
            ) {
                if (vars.dev.value) {
          ...
          vars[axis].value !== vars.time.value
                    ...
                    vars[axis].ticks.values = uniques(
                        vars.axes.dataset,
                        vars[axis].value,
                        fetchValue,
                        vars
                ...
                zero =
          opts.zero === true || axis.indexOf(opts.zero) === 0 ? true : false;
