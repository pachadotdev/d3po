...
find = (vars, node, variable, depth) ->
  ...
  if nodeObject
    ...
    if variable of node.d3po.data[cache]
...
cacheInit = (node, cache, vars) ->
  node.d3po = {} unless "d3po" of node
  node.d3po.data = {} unless "data" of node.d3po
  if vars.data.changed or (vars.attrs and vars.attrs.changed) or !(cache of node.d3po.data)
