...
module.exports = function(vars) {
  ...
  function searchStyle(elem) {
    elem
      ...
      .style(
        'background-color',
        d3
          .rgb(vars.ui.color.primary.value)
          .darker(0.15)
          .toString()
      );
  }
  ...
  function inputStyle(elem) {
    ...
    elem
      .style(
        'padding',
        vars.ui.padding.left / 2 + vars.ui.padding.right / 2 + 'px'
      )
      ...
      .attr(
        'placeholder',
        vars.format.value(vars.format.locale.value.method.search)
      );
  }
  ...
  if (vars.draw.timing) {
    ...
    vars.container.search.call(searchStyle);

    vars.container.search.select('input').call(inputStyle);
  }
  ...
  vars.search.height = vars.search.enabled
    ? vars.container.search.node().offsetHeight ||
      vars.container.search.node().getBoundingClientRect().height
    : 0;
