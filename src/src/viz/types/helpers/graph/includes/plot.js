(function() {
  ...
  module.exports = function(vars, opts) {
    ...
    for (j = 0, len = axes.length; j < len; j++) {
      ...
      if (vars[axis].value) {
        if (vars[axis].ticks.values === false) {
          if (vars[axis].value === vars.time.value) {
            ...
            while (tick < extent[1]) {
              ...
              tick = new Date(
                newtick['set' + step](newtick['get' + step]() + 1)
              );
              ticks.push(tick);
            }
        ...
        if (
          vars[axis].ticks.values.length === 1 ||
          (opts.buffer &&
            opts.buffer !== opp &&
            axis === vars.axes.discrete &&
        ...
        if (vars[axis].value === vars.time.value) {
          ...
          if (vars[axis].ticks.value) {
            ...
            vars[axis].ticks.visible = vars[axis].ticks.labels.value.map(
              Number
            );
          } else {
    ...
    if (vars.small) {
      ...
      vars.axes.height -=
        vars.axes.margin.viz.top + vars.axes.margin.viz.bottom;
  ...
  labelPadding = function(vars) {
      ...
      if (vars[axis].value) {
        ...
        if (vars[axis].ticks.labels.value) {
          ...
          if (xValues.length === 1) {
            ...
            xMaxWidth =
              vars[axis].scale.viz(xValues[1]) -
              vars[axis].scale.viz(xValues[0]);
    ...
    if (y2Domain) {
      return vars.y2.scale.viz.range(
        buckets([0, vars.axes.height], y2Domain.length)
      );
    }
  };
