...
module.exports = (vars) ->
  ...
  style = (line, axis) ->
    if axis.indexOf("y") is 0
      line
        .attr "x1", -2
        .attr "x2", -8
        .attr "y1", (d) -> d.d3po.y - margin.top
        .attr "y2", (d) -> d.d3po.y - margin.top
    else
      line
        .attr "x1", (d) -> d.d3po.x - margin.left
        .attr "x2", (d) -> d.d3po.x - margin.left
  ...
  ticks = vars.group.select("g#d3po_graph_plane")
    .selectAll "g.d3po_data_tick"
    .data data, (d) ->
      mod = if axes.discrete then "_"+d.d3po[axes.discrete] else ""
      "tick_" + d[vars.id.value] + "_" + d.d3po.depth + mod
  ...
  for axis in ["x", "y"]
    ...
    tick = ticks.selectAll "line.d3po_data_"+axis
      .data axisData, (d) ->
        "tick_" + d[vars.id.value] + "_" + d.d3po.depth
