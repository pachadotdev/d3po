...
#' @return d3po widget
mod_geomap3_plot <- function(data = NULL) {
  ...
  d3po(dout, width = 800, height = 600) %>%
    po_geomap(daes(group = .data$id, size = .data$pokemon_count, tooltip = .data$name),
...
mod_geomap3_ui <- function(id) {
  ns <- NS(id)
  tagList(d3po::d3po_output(ns("plot"), width = "100%", height = "600px"))
}

mod_geomap3_server <- function(id, data = NULL) {
