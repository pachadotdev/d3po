# Module: treemap_onelevel
#' Treemap: Share of Pokemon by main type (squarify)
#' @param data data.frame
#' @return d3po widget
mod_treemap_onelevel_plot <- function(data = d3po::pokemon) {
...
mod_treemap_onelevel_ui <- function(id) {
  ns <- NS(id)
  tagList(d3po::d3po_output(ns("plot"), width = "100%", height = "600px"))
}

mod_treemap_onelevel_server <- function(id, data = d3po::pokemon) {
  moduleServer(id, function(input, output, session) {
    output$plot <- d3po::render_d3po({
      mod_treemap_onelevel_plot(data)
    })
  })
}
