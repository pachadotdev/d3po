...
module.exports = function(params) {
  ...
  if (params.fullscreen) {
    ...
    params.x = params.parent
      ? params.parent.node().offsetWidth / 2
      : window.innerWidth / 2;
  ...
  if (params.fullscreen && params.html && !params.stacked) {
    w = params.parent
      ? params.parent.node().offsetWidth * 0.75
      : window.innerWidth * 0.75;
    h = params.parent ? parentHeight * 0.75 : window.innerHeight * 0.75;

    container.style('width', w + 'px').style('height', h + 'px');
  ...
  if (!params.mouseevents) {
    ...
    var newout = function() {
      ...
      if (
        !target ||
        (!ischild(tooltip.node(), target) &&
          !ischild(params.mouseevents, target) &&
          !istooltip)
      ) {
  ...
  if (params.data) {
    ...
    params.data.forEach(function(d, i) {
      ...
      if (d.link) {
        name.style('cursor', 'pointer').on(events.click, d.link);
      }
  ...
  if (params.html && (!params.fullscreen || params.stacked)) {
    data_container.append('div').html(params.html);
    if (params.js) {
      params.js(container);
    }
  }
  ...
  if (params.data || ((!params.fullscreen || params.stacked) && params.html)) {
    ...
    if (footer) {
      h -=
        footer.node().offsetHeight ||
        footer.node().getBoundingClientRect().height;
      h -= parseFloat(footer.style('padding-top'), 10);
      h -= parseFloat(footer.style('padding-bottom'), 10);
    }

    data_container.style('max-height', h + 'px');
  }
